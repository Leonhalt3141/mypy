...
class GImage(DSImage):
    ...
    def Band2Array(self, bn):
        ...
        array = band.ReadAsArray().reshape(cols * rows)
        return array
    ...
    def Array2Tiff(self, out_name, Array, fill=numpy.nan):
        cols = self.iminfo['cols']
        rows = self.iminfo['rows']
...
class MultiImages:
    ...
    def CalcSD(self):
        ...
        return numpy.sqrt(numpy.sum((AllArrays - AverageArrays) * (AllArrays - AverageArrays), axis=0) / n)
